...
namespace Project_4A
{
    public partial class frmMST : Form
    {
        ...
        {
            theGraph = g;
            int [,] mstAdjMat = new int[g.vertexList.Length,g.vertexList.Length];
            for (int i = 0; i < mstAdjMat.GetLength(0); i++)
            {
                for (int j = 0; j < mstAdjMat.GetLength(1); j++)
                {
                    mstAdjMat[i, j] = INFINITY;
                }
            }
            g.mstw(mstAdjMat);
            ...
            AyritlariYerlestir(listOvalVertex, mstAdjMat, canvas);
            InitializeComponent();
            lblAgirlik.Text += agirlik;
        }
